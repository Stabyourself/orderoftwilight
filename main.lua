function love.load()
	...
	tilewidth = 6
	scale = 4
	if love.graphics.getMode() ~= width*tilewidth*scale then
		love.graphics.setMode(width*tilewidth*scale, height*tilewidth*scale, false, false, 0)
	end
	love.graphics.setIcon(love.graphics.newImage("graphics/icon.gif"))
...
function love.draw()
	...
	love.graphics.setColor(255, 255, 255, 2.55*noise)
...
function loadhighscores()
	if love.filesystem.exists("high") then
...
function numberprint(s, x, y)
	...
	for j = 1, #s do
		...
		if i then
			love.graphics.drawq(numberfontimg, numberfontquads[i], math.floor((x+(j-1)*5)*scale), math.floor(y*scale), 0, scale, scale)
		end
	end
end
	...
	for j = 1, #s do
		...
		if background then
			...
			if pos > -xscroll-4 and pos < xscroll+width*tilewidth+8 then
				love.graphics.drawq(fontbackimg, fontbackquads[i], math.floor((pos)*scale), math.floor((y-1)*scale), 0, scale, scale)
			end
		else
			love.graphics.drawq(fontimg, fontquads[i], math.floor((x+(j-1)*8)*scale), math.floor(y*scale), 0, scale, scale)
		end
	end
end
...
function properprintsmall(s, x, y)
	for j = 1, #s do
		local i = string.sub(s, j, j)
		love.graphics.drawq(smallfontimg, smallfontquads[i], math.floor((x+(j-1)*6)*scale), math.floor(y*scale), 0, scale, scale)
	end
end
